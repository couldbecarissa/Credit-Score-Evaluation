...
weight_CreditUtilization=0.25
weight_PaymentHistory=0.5
weight_MaturityIndex=0.05
weight_LoanTerm=0.1
weight_CreditAccounts=0.1

#Finding a credit utilization score
def CreditUtilization(financial_literacy,total_amount_in_debt,C7,B15,C3,PPIscore):
    cred=0
    ...
    if(financial_literacy=="yes"):
        cred+=0.1
         ...
         cred+=0
        #weight for total amount in debt=40%,remaining=50%
    weight_amount_in_debt = 0.4
    cred+=normalization.normalize_debt(debt=total_amount_in_debt)*weight_amount_in_debt

    #weight for customer payment method=15%,remaining=35%
    weight_payment=0.15
    cred+=normalization.normalize_payment_methods(C7=C7)*weight_payment

    #weight for the  PPI index takes up 20%
    weight_PPI=0.2
    cred+=weight_PPI*normalization.normalize_PPI_score(PPIscore=PPIscore)

    #last 15%for housing issues and situations
...
def LoanTerm(loan_term,market):
    cred=0
    #here the period for payment has the weight of 60%
    weight_payback_period=0.6
    if(loan_term<=7):
    ...
    #weight for the marketplace location is 40%
    weight_market=0.4
    cred+=weight_market*normalization.normalize_market(market=market)
    return cred
...
def calculate_credit_score(user):
            ...
            loan_terms=LoanTerm(loan_term=user.get("loan_term",7),#Default loan term is  7 days
                                market=user.get("market","Makumbusho"))#Default value when no market is added is "Makumbusho"
